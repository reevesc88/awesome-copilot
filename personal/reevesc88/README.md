...
6. Open the copied `.github/copilot-instructions.md` and fill in all `{{PLACEHOLDER}}` values.
7. Decide which workflows should stay template-only and which should be compiled and enabled.
8. In the target repo, compile workflow source files into `.lock.yml` files if you are using `gh aw`.
9. Commit changes on a branch and open a **draft PR** for human review.
